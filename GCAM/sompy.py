...
class SOM(object):
    ...
    def train(self, trainlen = None, n_job = 1, shared_memory = 'no',verbose='on'):
        ...
        if verbose=='on':
            print
        batchtrain(self, njob = n_job, phase = 'rough', shared_memory = 'no',verbose=verbose, trainlen=trainlen)
        if verbose=='on':
            print
        #######################################
        #Finetuning
        if verbose=='on':
            print
        batchtrain(self, njob = n_job, phase = 'finetune', shared_memory = 'no',verbose=verbose, trainlen=trainlen)
...
#             print
    ...
    #     		plt.scatter(X, msz[0]-1-Y, s=area, alpha=0.9,c='None',marker='o',cmap='jet',linewidths=3, edgecolor = 'r')
            ...
            plt.ylim(0,msz[0])
        plt.tight_layout()
        plt.savefig(os.path.join(path, 'GCAM_hitmap.png'))
        plt.clf()
...
def batchtrain(self, njob = 1, phase = None, shared_memory = 'no', verbose='on', trainlen = None):
    t0 = time()
    nnodes = getattr(self, 'nnodes')
    dlen = getattr(self, 'dlen')
    dim = getattr(self, 'dim')
...
def view_2d(self, path, text_size, which_dim='all', what = 'codebook'):
    msz0, msz1 = getattr(self, 'mapsize')
    if what == 'codebook':
...
#             pl = plt.imshow(mp[::-1])
            ...
            plt.colorbar(pl)
        plt.tight_layout()
        plt.savefig(os.path.join(path, '2d_map.png'))
        plt.clf()
